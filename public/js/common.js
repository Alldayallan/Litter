...
$("#deletePostButton").click((event) => {
    ...
    $.ajax({
        ...
        success: (data, status, xhr) => {
            ...
            if(xhr.status != 202) {
                alert("Whoopsie! You can only delete your own posts");
                return;
            }
...
function createPostHtml(postData, largeFont = false) {
    ...
    if (postData.postedBy._id == userLoggedIn._id) {
        ...
        if (postData.pinned === true) {
            ...
            pinnedPostText = "<i class='far fa-thumbtack'></i> <span>Pinned post</span>";
        }

        buttons = `<button class='pinButton ${pinnedClass}' data-id="${postData._id}" data-toggle="modal" data-target="${dataTarget}"><i class='fal fa-thumbtack'></i></button>
                    <button data-id="${postData._id}" data-toggle="modal" data-target="#deletePostModal"><i class='fal fa-times'></i></button>`;
    }
...
function getNotificationText(notification) {
    ...
    if(notification.notificationType ==  "retweet") {
        text = `${userFromName} recycled one of your posts`;
    }
